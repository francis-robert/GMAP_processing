...
#'
#' @return a dataframe of analytes flagged according to concentrations of threshholds defined by the Minimum Detection Limit, and high span
#' @export
#'
#' @examples
analyte_flagging <- function(path, onoff = " ", trantype = " ",
                             h2smdl = NULL, h2shs = NULL, ch4mdl = NULL,
                             ch4hs = NULL, benmdl = NULL, benhs = NULL,
                             tolmdl = NULL, tolhs = NULL, xypmdl = NULL,
                             xyphs = NULL) {
  ...
  if (onoff == "off") {
    if (trantype == "MA")  {
      ...
      input_2 <- input %>%
        ...
        dplyr::rename(., "H2S(ppb)" = "H2S", "CH4(ppm)" = "CH4",
                      ...
                      "XYP(ppb)" = "XYP", "Latitude" = "GPSLatitude",
                      "Longitude" = "GPSLongitude",
                      "ws" = "AirMar.Wind.Speed..m.s.",
                      "wd" = "AirMar.Wind.Direction..TRUE.") %>%
        relocate("Time", "Transect", "H2S(ppb)", "H2Sflag", "CH4(ppm)",
                 "CH4flag",
                 "BEN(ppb)", "BENflag", "TOL(ppb)", "TOLflag", "XYP(ppb)",
                 "XYPflag",  "Latitude", "Longitude", "ws", "wsflag", "wd",
                 "wdflag") %>%
        mutate(`H2S(ppb)` =
                 replace(`H2S(ppb)`,
    ...
    if (trantype == "CA") {
      files <- list.files(path = path, pattern = "CA", full.names =T)
      print(files)
      input_ca <- lapply(files, FUN = read.table,
                         skip = 25, header = TRUE, sep = "\t",
                         fill = TRUE,
                         stringsAsFactors = FALSE, na.strings = "NaN",
                         col.names = c("Time", "CO2", "CH4", "H2S", "C2H2",
                                       "M3DSU", "M3DSV", "M3DSW",
        ...
        mutate(TOLflag = "AZ") %>%
        mutate(XYPflag = "AZ") %>%
        rename("H2S(ppb)" = "H2S", "CH4(ppm)" = "CH4", "BEN(ppb)" = "BEN",
               "TOL(ppb)" = "TOL", "XYP(ppb)" = "XYP") %>%
        relocate("Time", "Transect", "H2S(ppb)", "H2Sflag", "CH4(ppm)", "CH4flag",
                 "BEN(ppb)", "BENflag", "TOL(ppb)", "TOLflag", "XYP(ppb)",
                 "XYPflag")
