...
syft_data_sub_pet <- subsamp_temporal_syft(syft_data_met,samp_int_local)
syft_data_sub_btex <- subsamp_temporal_syft(syft_data_met,samp_int_local)
x<-output_csv_data(pic_data_sub,output_type = ("picarro"),write=F)
y<-output_csv_data(syft_data_sub,output_type = "syft",write=F)
...
timeseries<-ts_plot(syft_data_sub,tran_time_minmax, mdl_df, grp = c("A","B","Y","Z","BleepBloop"),
                    analyte=c("benzene,dbe,mek,pce","toluene,xyleth,trimethyl", "chbr3,ch3br","H2S","heptane,hexane,toluene"),
                    time_labels="30",time_labels_graph = "30 sec")

timeseries_single<-ts_plot(syft_data_sub,tran_time_minmax,mdl_df, analyte = "benzene",grp = "A",
                           multi_analyte = F,proc="petroleum",time_labels="30",
                           time_labels_graph = "30 sec")
...
MA_test_2<-MA_map(x= syft_data_sub,
                  rast_path = "C:/Users/rfranc01/OneDrive - Environmental Protection Agency (EPA)/Downloads/landsat_2",
                  z=break_pt,
                  analyte = "benzene",
                  extent = "w",
                  transect= "250507_MA14",campaign = "test_test",
                  rast_type = "landsat",pt_size = 2,color_pal = "b",zoom_scale = 500)
